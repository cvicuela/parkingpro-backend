-- Migration 051: Fix tolerance not being subtracted in process_parking_payment
-- Bug: When parking duration exceeds tolerance, hours were calculated from
-- total minutes WITHOUT subtracting tolerance first.
-- Example: 63 min with 5 min tolerance → CEIL(63/60)=2h (WRONG)
-- Fixed:  63 min with 5 min tolerance → CEIL((63-5)/60)=1h (CORRECT)
-- Also fixes: uses session's plan_id instead of generic 'hourly' plan lookup
...
BEGIN
  ...
  IF v_total IS NULL THEN
    -- Use session's plan_id if available, otherwise fall back to generic hourly plan
    ...
    IF v_minutes <= v_tolerance THEN
      ...
      v_hours := 0;
    ELSE
      -- FIX: Subtract tolerance before calculating billable hours
